...
#INTERVALO DE TIEMPO
next_send, send_interval = 0, 60 #segundos
#FUNCIONES
def actualizar_next_send(tiempo_actual):
    return tiempo_actual + send_interval #en segundos
...
def apagar_todo(led_temp,led_hum,led_lum,led_hs,oled):
    ...
    led_hs.apagar_todos()
    oled.apagar()
def conteo_regresivo(siguiente, actual):
    diferencia = siguiente - actual
    if diferencia>=0:
        print(f"Se enviará la data a DB y Telegram en: {diferencia} segundos")
#BUCLE PRINCIPAL
def main_loop():
    update_id = 0
    mqtt.setup_mqtt_subscription()
    ultimo_intento_reconexion = utime.time()
    next_send = actualizar_next_send(utime.time())
    while True:
        try:
            ...
            # Actualizar valores de sensores para mostrarlo en pantalla oled
            ...
            # Procesar mensajes de Telegram si hay alguno (ejm: comando /start)
