...
send_interval = 0.4 # Intervalo de envío de mensajes en segundos, 900 son 15 minutos, lo pongo a 2 para probar
...
def read_message(bot, update_id):
    gc.collect()  # Intenta liberar memoria antes de realizar la operación
    try:
        url = f"{API_URL}/getUpdates?limit=1&offset={update_id}"
        response = urequests.get(url)
        data = response.json()  # Procesa la respuesta
        response.close()  # Asegura que la respuesta se cierra para liberar recursos
    except OSError as e:
        if e.args[0] == 12:  # ENOMEM
            print("Error: No hay suficiente memoria para leer el mensaje de Telegram.")
            gc.collect()  # Intenta liberar memoria después de un error
            return update_id, None
        print("Error de conexión al leer mensaje de Telegram:", e)
        return update_id, None
    ...
    if 'result' in data and data['result']:
        ...
        return update_id, update['message']
    else:
        return update_id, None
...
def enviar(fecha, hora, temperatura, humedad, intensidad_luz, humedad_suelo):
    ...
    try:
        ...
        if e.args[0] == 104:  # ECONNRESET
            print("Error de conexión MQTT: El servidor ha cerrado la conexión.")
        elif e.args[0] == 110:  # ETIMEDOUT
            print("Error de conexión MQTT: Tiempo de espera agotado.")
        else:
            print(f"Error de conexión MQTT: {e}")
    except Exception as e:
        print(f"Error inesperado al enviar datos por MQTT: {e}")
...
def update_sensor_data():
    ...
    raw_humedad_suelo = sensor_humedad_suelo.read_u16()
    
    # Asegúrate de que los valores negativos se conviertan en 0
    if raw_humedad_suelo < 0:
        raw_humedad_suelo = 0
    ...
    # Asegurarse de que el valor calculado no sea negativo
    porcentaje_humedad_suelo = max(0, round((minima_humedad - raw_humedad_suelo) * 100 / (minima_humedad - maxima_humedad), 2))
...
def main_loop():
    ...
    next_send = utime.time() + send_interval
    gc_interval = 30  # Intervalo para ejecutar gc.collect(), en segundos.
    last_gc_run = utime.time()
    ...
    while True:
        try:
            ...
            if e.args[0] in [110, 104, -1]:  # Captura errores específicos
                print("El servidor no está disponible. Por favor, inténtelo más tarde.")
            else:
